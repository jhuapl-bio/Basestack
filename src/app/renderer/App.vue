  ...
  <main id="app" class="subpixel-antialiased bg-white font-body">
    ...
    >
      <header class="px-24 py-12 space-y-2 border border-white rounded-lg">
        <h2 class="text-center text-white uppercase markup-h5">
          Welcome to Basestack
        </h2>
        <h3 class="text-center text-white markup-h2 animate-pulse">
          Initiating…
        </h3>
      </header>
    </div>
    ...
    <div v-else>
      <app-layout hasSidebar="true">
        <template #title>
          <h3 class="text-gray-800 markup-h3">
      ...
      this.interval = setInterval(async () => {
        if (this.runningServer) {
          ...
          try {
            ...
            console.error(
              "Could not get server status, check if it is running on port: ",
