  ...
  <app-layout>
    <template #title>
      <div class="flex items-center justify-between">
        <h2 class="text-blue-900 markup-h1">Library</h2>
        ...
        <tabs>
          <tab name="My Procedures" class="flex flex-col space-y-12">
            <div class="px-6 py-4 bg-blue-100 rounded-lg">
              ...
              <div class="flex items-center justify-between">
                <p class="markup-body-sm">
                  String together services across modules to create a custom
                  procedure
                </p>
          ...
          <tab name="My Modules" class="flex flex-col space-y-12">
            ...
            <section class="grid grid-cols-3 gap-4">
              <module-library-card
                v-for="(module, index) in catalog.filter(
                  (module) => module.status.installed
                )"
                :key="module.name"
          ...
          <tab name="Browse All Modules" class="flex flex-col space-y-12">
            <h4 class="text-blue-900 markup-h4">Browse All Modules</h4>
            <!-- @todo: this is tied to the active tab -->

            <section class="grid grid-cols-3 gap-4">
              <!-- @todo: temp made description a list of the module tags. figure out what the description will actually be -->
              <module-library-card
                v-for="(module, index) in catalog"
                :key="module.name"
                :moduleName="module.title"
                :version="module.latest_version"
                :description="module.tags.join(' | ')"
