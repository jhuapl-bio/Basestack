  ...
  <v-row>
    <v-col sm="12">
      <v-toolbar cyan width="100%" class="elevation-12">
        ...
        >
          <v-icon class="mr-3" small> $cog </v-icon>
          Customize
        </v-btn>
        ...
        >
          <template v-slot:selection="{ item }">
            <v-list-item-title>{{
              item.version != latest ? "Not Latest" : "Latest"
            }}</v-list-item-title>
            <v-list-item-subtitle>v{{ item.version }}</v-list-item-subtitle>
          </template>
          <template v-slot:item="{ item }">
            <v-list-item-avatar>
              <v-icon v-if="item.version == latest" small color="green"
                >$check-circle
              </v-icon>
            ...
            <v-list-item-content>
              ...
              <v-list-item-subtitle
                v-text="
                  !item.imported && !item.local ? 'Not Imported' : 'Local'
                "
              ></v-list-item-subtitle>
            </v-list-item-content>
            <v-list-item-action>
          ...
          <template v-slot:append>
            <v-tooltip top class="ml-2">
              <template v-slot:activator="{ on }">
                <v-icon v-on="on" @click="importVersion" medium color="primary"
                  >$download</v-icon
                >
              </template>
        ...
        <template v-slot:extension>
          ...
          <v-btn color="primary" class="text-caption" @click="reset()">
            <v-icon class="mr-3" small color="primary lighten-2"> $cog </v-icon>
            Reset Default</v-btn
          >
        </template>
        <v-tooltip
          top
          class="ml-2"
          :key="`${selectedVersion.imported}-importedkey`"
          v-if="selectedVersion.removable && selectedVersion.imported"
        >
          <template v-slot:activator="{ on }">
            <v-btn
              @click="removeModule(selectedVersion.idx, selectedVersion.name)"
              v-on="on"
              icon
            >
              <v-icon color="orange" medium> $trash-alt </v-icon>
            </v-btn>
          </template>
          Remove saved module
        </v-tooltip>
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-icon
              ...
              medium
              color="indigo "
              v-on:click="fetchRemoteCatalog(selectedVersion.name)"
              style="text-align: right"
              class="configure ml-2 mr-3"
              >$external-link-alt</v-icon
            >
          </template>
        ...
        <v-tooltip top>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" icon @click="start_procedure()">
              <v-icon color="primary" medium> $play-circle </v-icon>
            </v-btn>
          </template>
        ...
        <v-tooltip top v-if="job && job.running">
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" medium icon @click="cancel_procedure()">
              <v-icon color="orange darken-2" medium> $times </v-icon>
            </v-btn>
          </template>
          Cancel Procedure
        </v-tooltip>
    ...
    >
      <v-list-item>
        <v-list-item-avatar>
          <v-icon color="primary">{{
            procedure.icon ? "$" + procedure.icon : "$cog"
          }}</v-icon>
        </v-list-item-avatar>
      ...
      <v-select :items="serviceList" v-model="el" label="Adjust service">
      </v-select>

      <v-card class="scroll fill-height">
        <v-subheader> Adjust Cmd in Docker Pipeline </v-subheader>
        <v-textarea
    ...
    <v-col sm="3" class="shrink">
      <v-dialog transition="dialog-bottom-transition">
        <template v-slot:activator="{ on, attrs }">
          ...
          >
            ...
            <v-card-title v-else class="text-h5"> Module Status </v-card-title>

            <v-card-subtitle
              >{{ showInstalled }} up-to-date dependencies</v-card-subtitle
            >
            <v-card-text v-if="installStatus.fully_installed"
              >All dependencies installed</v-card-text
            >
            <v-card-text v-else>Missing 1 or more dependencies</v-card-text>
            <v-card-actions>
              <v-btn v-on="on" v-bind="attrs" class="text-caption" dark small>
                Check
                <looping-rhombuses-spinner
                  :animation-duration="6000"
                  v-if="installStatus.building"
                  :size="3"
                  class="ml-1"
                  :color="'white'"
                />
              ...
              >
                Build
                <v-icon small class="ml-3"> $download </v-icon>
              </v-btn>
            </v-card-actions>
          </v-card>
          <!-- <v-alert  prominent 
      ...
      <v-alert
        ...
        type="info"
      >
        This procedure utilizes a rendered UI, select the "Click Me" button
        below to open it
      </v-alert>
      <v-banner v-if="selectedVersion.version != latest">
        <v-tooltip bottom class="">
          <template v-slot:activator="{ on }">
            <v-icon class="ml-3" v-on="on" color="warning" medium>
              $exclamation-triangle
            </v-icon>
          </template>
          View the Library to install version {{ latest }} or select from the
          drop-down if already loaded
        </v-tooltip>
        {{
          latest
    ...
    <v-col sm="9">
      <v-subheader
        class="overflow-x-visible mx-4 indigo lighten-5"
        v-if="selectedVersion.description"
        >{{ selectedVersion.description }}</v-subheader
      >
      <v-stepper v-model="el" class="pb-0" v-if="services">
        <v-stepper-header class="configure">
          <template v-for="(entry, key) in services">
            <v-stepper-step
              ...
              :rules="[
                () => {
                  return status[key] && status[key].error
                    ? !status[key].error
                    : true;
                },
              ]"
            ...
            >
              ...
              <small class=""
                >{{
                  services_to_use[key] >= 1 || !services_to_use[key] == null
                    ? "Enabled"
                    : "Disabled"
                }}
                <v-tooltip
                ...
                <v-tooltip
                  top
                  v-if="status && status[key] && status[key].error"
                >
                  <template v-slot:activator="{ on }">
                    <v-icon
                      v-on="on"
                ...
                <v-tooltip
                  top
                  v-else-if="status && status[key] && status[key].cancelled"
                >
                  <template v-slot:activator="{ on }">
                    <v-icon
                      v-on="on"
                    ...
                    <v-card>
                      ...
                      <v-card-text>
                        ...
                        <v-text-field
                          ...
                          single-line
                        ></v-text-field>
                        <v-btn text @click="custom_images[key] = entry.image"
                          >Default</v-btn
                        >
                      </v-card-text>
    ...
    <v-col
      sm="6"
      v-if="procedure && procedure.variables"
      class="overflow:auto; min-height:"
    >
      <ListParams
        :items="additionals"
...
export default {
  ...
  methods: {
    ...
    async fetchRemoteCatalog(name) {
      const $this = this;
      FileService.fetchRemoteCatalog("stagedModules", name)
        .then((f) => {
          this.getLibrary()
            .then((f) => {
    ...
    reset() {
      ...
      let found = nestedProperty.get(
        this.$store.state,
        `configs.${this.selectedVersion.name}.procedures.${this.procedureIdx}`
      );
      if (found) {
    ...
    loadProcedure(proc) {
      ...
      let found = nestedProperty.get(
        this.$store.state,
        `catalog.${this.selectedVersion.name}.procedures.${this.procedureIdx}`
      );
  ...
  watch: {
    ...
    selected(newValue) {
      ...
      this.installStatus = {};
      if (
        this.selectedVersion.procedures &&
        this.selectedVersion.procedures[0]
      ) {
  ...
  computed: {
        ...
        for (let [key, value] of Object.entries(this.procedure.variables)) {
          if (!value.output) {
            ...
            if (!value.optionValue && value.options)
              [
                (value.optionValue = value.option
    ...
    selected_procedure_index() {
      if (this.module && this.selectedVersion.name) {
        let index = this.selectedVersion.procedures.findIndex(
          (data) => data === this.procedure
        );
        if (index == -1) {
          return 0;
        } else {
