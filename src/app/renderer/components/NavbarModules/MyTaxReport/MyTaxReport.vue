  ...
  <div id="mytaxreport" >
	  <b-row>
		<b-col sm="6">
			...
			<div v-if="cmd">
				<b-form-group
					v-for="dependency in cmd.dependencies"
					:key="dependency.key"
					:label="dependency.key"
				>
					<div v-if="dependency.type == 'file'">
						<b-form-file 
							v-model="staged[dependency.key]"
							:placeholder="'Choose a fastq file'"
							drop-placeholder="Drop file here..."
						>
						</b-form-file>
					</div>
					<div v-else-if="dependency.type == 'dir'">
						{{dependency}}
					</div>
	    ...
	    <!-- <b-col sm="6" >
			<b-table
				v-if="module_statuses.dbs"
				small
				id="module_table"
				class="formGroup-input"
				:items="module_statuses.dbs"
				sticky-header="700px"						        
			>	
			...
			<b-table
				v-if="module_statuses.report"
				small
				id="module_table2"
				class="formGroup-input"
				:items="module_statuses.report"
				sticky-header="700px"						        
			>	
				<template  v-slot:cell(status)="row">
					<font-awesome-icon :class="[ 'text-success' ]" 
