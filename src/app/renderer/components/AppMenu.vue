<template>
    <nav class="min-w-[123px] h-screen grid grid-cols-1 grid-rows-8 border-r border-l border-gray-300">
        ...
        <section class="flex flex-col items-start w-full overflow-y-auto duration-500 hide-scrollbar"
            :class="[showAllModules ? 'row-span-6 h-full' : 'row-span-3 h-[50%]']">
            <div ref="modulesContainer" class="w-full px-4 pt-6 hide-scrollbar"
                :class="[showAllModules ? 'overflow-y-auto' : 'overflow-y-hidden']">
                <div class="grid w-full grid-cols-1 gap-2 place-items-center">
                    <div v-for="(module, index) in installedModules" :key="index">
                        <module-button :moduleName="module.name" />
                    </div>
                </div>
            </div>

            <button @click="toggleShowingAllModules"
        ...
        <section class="mt-6 duration-500 origin-bottom border-t border-gray-400 divide-y divide-gray-400"
            :class="[showAllModules ? 'opacity-0 scale-0 hidden' : 'opacity-100 row-span-3 scale-100']">
