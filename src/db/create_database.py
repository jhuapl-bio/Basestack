...
parser.add_argument('--name', required = False, default="basestack", type=str,  help = 'output.json file')
parser.add_argument('--collection', required = False, default="stagedModules", type=str,  help = 'collection to push to ')
    ...
    if config: 
        for  item in config['modules']:
            ...
            if 'recursive' in item and item['recursive']:
                files = list(Path(item['path']).rglob("*."+formatted))
            else:
                files = [os.path.join(specification['path'], f) for f in os.listdir(item['path']) if f.endswith(formatted)]
            if "ignores" in item:
                files = [f for f in files if os.path.basename(f) not in item['ignores']]
            for f in files:
