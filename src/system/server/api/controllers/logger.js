...
const { system } = require("../store/index.js")
...
export const createLoggingObject = function( name ){
  let loggingObject = createLogger({
    ...
    transports: [
      ...
      // - Write all logs error (and below) to `error.log`.
      //
      new transports.File({ filename: path.join(system.logFolder, `${name}.log`), maxsize: 1000000, maxFiles: 1,  tailable: true, options: { flags: 'a' } })
    ]
  })
