...
router.post("/workflow/build", (req,res,next)=>{ // build workflow according to name and index
	(async function(){
		try {
			let orchestrator = store.modules[req.body.name].services[req.body.service]
			let response = await orchestrator.build()
			console.log(response)
			logger.info(`Success in beginning to build workflow: ${req.body.name}`)
...
router.post("/workflow/run", (req,res,next)=>{ // build workflow according to name and index
	(async function(){
		try {
			let orchestrator = store.modules[req.body.name].services[req.body.service]
			let response = await orchestrator.run(req.body.data)
			logger.info(response)
			res.status(200).json({status: 200, message: "Completed workflow run start", data: response });
		} catch(err2){
...
router.get("/workflows/status", (req,res,next)=>{ 
	try {
		let modules = Object.keys(store.modules)
		let running = [];
		modules.forEach((module)=>{
			let orchestrator = store.modules[module]
			orchestrator.services.forEach((service)=>{
				running.push(
					{
						name: service.name,
